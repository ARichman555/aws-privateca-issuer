...
func getCaArn(caType string) string {
	switch caType {
	case "RSA":
		return testContext.rsaCaArn
	case "ECDSA":
		return testContext.ecCaArn
	case "XA":
		return testContext.xaCaArn
	default:
...
func getCertSpec(certType string) cmv1.CertificateSpec {
	...
	case "SHORT_VALIDITY":
		return getCertSpecWithValidity("RSA", 20, 5)
	case "CA":
		return getCaCertSpec("RSA")
	default:
		panic(fmt.Sprintf("Unknown Certificate Type: %s", certType))
	}
}

func getCertSpecWithValidity(certType string, duration time.Duration, renewBefore time.Duration) cmv1.CertificateSpec {
	certSpec := getBaseCertSpec(certType)
...
func getCaCertSpec(certType string) cmv1.CertificateSpec {
	certSpec := getBaseCertSpec(certType)
	certSpec.IsCA = true
	certSpec.Duration = &metav1.Duration{
