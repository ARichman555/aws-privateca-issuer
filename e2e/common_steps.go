...
func (issCtx *IssuerContext) issueCACertificate(ctx context.Context, keyAlgorithm string) error {
	certConfig := defaultCertConfig()
	certConfig.IsCA = true
	certConfig.KeyAlgorithm = keyAlgorithm

	return issCtx.issueCertificate(ctx, certConfig)
}

func (issCtx *IssuerContext) issueCertificateWithKeyAlgorithm(ctx context.Context, keyAlgorithm string) error {
	certConfig := defaultCertConfig()
	certConfig.KeyAlgorithm = keyAlgorithm
...
func (issCtx *IssuerContext) issueCertificateWithUsage(ctx context.Context, keyAlgorithm string, usageStr string) error {
	certConfig := defaultCertConfig()
	certConfig.KeyAlgorithm = keyAlgorithm
	certConfig.Usages = parseUsages(usageStr)

	return issCtx.issueCertificate(ctx, certConfig)
}

func (issCtx *IssuerContext) issueCertificateWithValidity(ctx context.Context, keyAlgorithm string, duration int, renewBefore int) error {
	certConfig := defaultCertConfig()
	certConfig.KeyAlgorithm = keyAlgorithm
	certConfig.Duration = time.Duration(duration)
	certConfig.RenewBefore = time.Duration(renewBefore)
