...
RUN --mount=type=cache,target=${go_mod_cache} --mount=type=cache,target=${go_cache} go build ./... 

ARG pkg_version

# Build
RUN --mount=type=cache,target=${go_mod_cache} --mount=type=cache,target=${go_cache} \ 
    VERSION=$pkg_version && \
    go build \
